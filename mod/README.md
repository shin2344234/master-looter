...
1. Take [docs/MasterLooter.template.txt](../docs/MasterLooter.template.txt), which holds all 454 strings. It is generated from the source by `scripts/make_translation_template.py`, so it covers the whole menu.
...
- Gather nodes are identified by their prefab, matched against a table of 2,019 node kinds built from the game's own data. A node outside that table is identified by what lands in your bag when it is gathered, and is otherwise left alone until you turn Unidentified nodes on.
...
Character Creator 9 hooks the same swapchain creation call as this plugin, from a thread that starts at the same moment, and its hook can land on top of this one. When that happens the menu gets no frame and Insert does nothing, while looting carries on. Since 1.6.42 the plugin watches for that and puts its hook back in front, which is logged as `wrote its own jump over this mod's CreateSwapChainForHwnd detour`. If Insert still does nothing, a `disable.txt` containing `overlay` in `bin64\CharacterCreator` switches that mod's editor panel off, and the `[creation]` lines in the log say which way the game made its swapchain.
