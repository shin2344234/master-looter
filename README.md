...
Four scanners out of seventy flag `MasterLooter.asi`: Microsoft (`Trojan:Win32/Wacatac.B!ml`), Symantec (`ML.Attribute.HighConfidence`), CrowdStrike Falcon (`win/malicious_confidence_60%`) and Cynet (`Malicious (score: 100)`). Three of them say what they are on the label, since `!ml`, `ML.Attribute` and a confidence percentage are all how a model reports a guess about the shape of a file. Cynet gives a score rather than naming anything, which is the same kind of answer worn differently. None of the four matched a known family, and the sixty-six others read the same file as clean. Both archives are clean.

The number moves release to release without the code changing character: 1.2.1 scored four, 1.4.0 four, 1.5.0 two, 1.5.1 four, 1.6.0 two, 1.6.1 three, 1.6.2 three, 1.6.3 three, 1.6.4 four. Microsoft and Symantec have flagged every one of them. Everything else comes and goes: Deep Instinct has left, CrowdStrike Falcon has stayed two releases, and Cynet appeared for the first time on a build whose only new code writes a diagnostic line to a log file. That is what a model guessing looks like, as against a scanner recognising something.
