...
Item, item group, drop set and drop source tables for Crimson Desert 2.01.00 (exe 1.0.0.2760), parsed from the game's own static-info files. This is the data layer behind the Master Looter plugin: it produces the item database and the creature table the plugin ships with.
...
3. Run, in order: `build_item_db.py`, `make_itemdb_tsv.py`, `make_creatures_tsv.py`. The drop set scripts (`build_dropsets.py`, `build_sources.py`) are only needed for the review pages.
